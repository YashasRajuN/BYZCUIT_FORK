...
ax0.errorbar(x, y, yerr=asymetricerrorStd, fmt='-o', label = "Clever_Input")
ax0.set_ylabel("Tps")
ax0.set_title("Evolution of the tps and latency in function of the nbr of shards")
ax0.grid(True, which = "both")

ax0.set_title("THREE NODES PER SHARD")

y = LatMean
asymetricerrorLatency = [LatStd, LatStd]
ax1.errorbar(x, y, yerr=asymetricerrorLatency, fmt='-o', label = "Clever_Input" )
...
ax0.errorbar(x, y, yerr=asymetricerrorStdRand, fmt='-x', label = "Random_input")

y = LatMeanRand
asymetricerrorLatencyRand = [LatStdRand, LatStdRand]
ax1.errorbar(x, y, yerr=asymetricerrorLatencyRand, fmt='-x', label = "Random_input")

ax0.legend()
ax1.legend()

#plt.savefig("Graph_Clever_Random_NODE_THREE.png")
...
ax[0].set_title("Distribution of the tps in function of the number of shard(s) used with THREE NODES")

for row in range(rows):
	nbr = len(allTps[shardList[row]-1][0])
	y = [0]*nbr
	ax[row].scatter(allTps[shardList[row]-1][0], y, label = 'Clever_Input')
	ax[row].set_ylabel('shard:'+str(shardList[row]))

	
	nbr = len(allTpsRand[shardList[row]-1][0])
	y = [0.5]*nbr	
	ax[row].scatter(allTpsRand[shardList[row]-1][0], y, label = 'random_input')
	ax[row].legend()
plt.xlabel("Tps")
#plt.savefig("Graph_distrib_Clever_Random_NODE_THREE.png")

arr =  [4224.7443737958265, 4062.4997935483307, 3493.4567495867923, 3374.8510334410676, 3088.4907505244823]
window_size = 2
